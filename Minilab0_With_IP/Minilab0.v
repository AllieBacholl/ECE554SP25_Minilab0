...
add_ip iadd(.clock(CLOCK_50), .aclr(~rst_n), .clken(&rden), .dataa({8'h00, multResult}), .datab(macout), .result(macout));
mult_ip imult(.dataa(dataout[0]), .datab(dataout[1]), .result(multResult));
